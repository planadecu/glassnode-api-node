 ...
 *
 * `null` and any other type (objects, arrays, bigints, …) are rejected. Rejections are a
...
export interface MetricParams {
  /** Asset symbol, e.g. `'BTC'`, `'ETH'` (`'*'` for all assets on bulk endpoints). */
  a?: string;
  ...
  c?: string;
  /** Exchange, for exchange-specific metrics, e.g. `'binance'`. */
  e?: string;
  ...
  api_key?: never;
  /** Any other parameter documented for the metric. `undefined` omits it. */
  [name: string]: MetricParamValue | undefined;
}
